...
export async function parseProjectData(inputDir, metadataOverride) {
    ...
    const total_transcripts = assembly_stats.find((s) => s.sample === 'merged.fasta')?.num_transcripts || 0;
    const mean_transcript_size = assembly_stats.find((s) => s.sample === 'merged.fasta')?.mean_size || 0;
    // Get DGE stats
...
async function loadStaticContent(scriptDir) {
...
async function validateDeliverablesStructure(inputDir) {
    const warnings = [];
    const requiredDirs = [
        '01_Raw_Data',
...
async function getSamples(inputDir) {
    const samples = [];
...
async function getSequencingStats(inputDir, samples) {
    const stats = [];
    const rawDataDir = path.join(inputDir, '01_Raw_Data');
    if (await fs.pathExists(rawDataDir)) {
        try {
...
async function getReferenceStats(inputDir) {
...
async function getMappingStats(inputDir, samples) {
    const stats = [];
...
async function getAssemblyStats(inputDir, samples) {
    const stats = [];
...
async function getDGEStats(inputDir) {
    const stats = [];
    const labels = [];
    const up_counts = [];
    const down_counts = [];
...
async function getPathwayStats(inputDir) {
    const stats = [];
...
async function getDGEFigures(inputDir) {
    const dgeDir = path.join(inputDir, '05_differential_expression_analysis');
    const figures = [];
    if (!await fs.pathExists(dgeDir)) {
        return figures;
    ...
    try {
        ...
        for (const file of files) {
            if (file.toLowerCase().includes('dge') && file.endsWith('.xlsx')) {
                ...
                let plotData = { ma: { up: [], down: [], ns: [] }, volcano: { up: [], down: [], ns: [] } };
                let hasMAData = false;
                ...
                try {
                    ...
                    if (!fcCol) {
                        fcCol = Object.keys(data[0]).find((c) => normCol(c).includes('logfc')) || null;
                        if (!fcCol) {
                            fcCol = Object.keys(data[0]).find((c) => normCol(c).includes('log2fold')) || null;
                        }
                    }
                    let cpmCol = findColumn(data, ['logcpm', 'log_cpm']);
                    if (!cpmCol) {
                        cpmCol = Object.keys(data[0]).find((c) => normCol(c).includes('logcpm')) || null;
                    }
                    let meanCol = findColumn(data, ['basemean', 'aveexpr']);
                    if (!meanCol) {
                        meanCol = Object.keys(data[0]).find((c) => normCol(c).includes('basemean')) || null;
                    }
                    let sigCol = findColumn(data, ['padj', 'adjp', 'adjpval', 'fdr', 'qvalue', 'qval', 'q_value', 'pvalue', 'pval', 'p_value']);
                    if (!sigCol) {
                        sigCol = Object.keys(data[0]).find((c) => {
...
function asFloat(v) {
    ...
    try {
        ...
        if (s === '' || s.toLowerCase() === 'na' || s.toLowerCase() === 'nan' || s.toLowerCase() === 'none')
            return null;
...
async function getDGEComparisonTables(inputDir) {
...
async function extractFunctionalAssets(inputDir) {
    ...
    async function findFiles(pattern, extensions) {
        const found = [];
    ...
    // Search for images
    async function findImages(pattern) {
        const found = [];
        async function search(dir) {
            if (await fs.pathExists(dir)) {
                const entries = await fs.readdir(dir, { withFileTypes: true });
...
async function getDeliverablesTree(inputDir) {
    ...
    if (await fs.pathExists(readmePath)) {
        try {
            const content = await fs.readFile(readmePath, 'utf-8');
            const lines = content.split('\n');
            const treeLines = lines.filter(line => line.includes('|--') || line.includes('├──') || line.includes('└──') || line.includes('│   ') || line.includes('--'));
...
async function findComponentAsset(scriptDir, assetName, inputDir) {
